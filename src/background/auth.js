...
export async function handlePostSignIn(isDiscordPage) {
    try {
        if (isDiscordPage) {
...
export async function setupSidePanel(tabId, enabled = true) {
    try {
...
async function authenticateWithBackend(accessToken) {
    ...
    try {
        ...
        if (tokenParts.length === 3) {
            ...
            console.log('Days until expiration:', Math.floor((payload.exp * 1000 - Date.now()) / (1000 * 60 * 60 * 24)));
        }
