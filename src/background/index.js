import { initializeGoogleAuth, handleSignOut, checkAuthStatus } from './auth';
...
async function broadcastToSidePanels(message) {
    // Get all tabs
...
async function setupSidePanel(tabId, enabled = true) {
    try {
        await chrome.sidePanel.setOptions({
            tabId,
            enabled,
...
async function navigateToDiscord(discordTabId) {
    try {
        if (discordTabId) {
...
async function handlePostSignIn(isDiscordPage) {
    try {
        if (isDiscordPage) {
